...
class IdleState(QState):
    ...
    def onEntry(self, event):
        try:
            ...
            logger.error(f"StateMachine IdleState onEntry error: {e}")

    def onExit(self, event):
        try:
            logger.debug("IdleState onExit")
            self.functions["disable_screen"]()
            return super().onExit(event)
        except Exception as e:
            logger.error(f"StateMachine IdleState onExit error: {e}")
...
class DisconnectionState(QState):
    ...
    def __init__(self, machine: QStateMachine | None, bluetoothHandle: BluetoothCommClass | None, btSerialHandle, functions = None):
        ...
        self.functions = functions
        self.sp_case = None#when a device is already connected and another device is trying to connect
        self.spp_counter = 0#for sp case, will go up to 2, represents the old device unpair and the new device pair
        self.hid_counter = 0#for sp case, will go up to 2, represents the old device unpair and the new device pair
    ...
    def onEntry(self, event):
        try:
            ...
            logger.error(f"StateMachine DisconnectionState onEntry error: {e}")
    ...
    def onExit(self, event):
        try:
            ...
            logger.error(f"StateMachine DisconnectionState onExit error: {e}")
    ...
    def handle_finish(self, status):
        try:
            ...
            logger.error(f"StateMachine DisconnectionState handle_finish error: {e}")

    #waits for both responses from hid and spp
    def sp_case_async_check(self, status):
        try:
            ...
            logger.error(f"StateMachine DisconnectionState sp_case_async_check error: {e}")
            raise

    #checks for connected devices and selected device 
    #if its disconnect only or disconnect and them connect new device
    def case_evaluation(self):
        try:
            ...
            logger.error(f"StateMachine DisconnectionState case_evaluation error: {e}")
            raise
    ...
    def conn_check(self):
        try:
            ...
            logger.error(f"StateMachine conn_check conn_check error: {e}")
            raise

    #waits for hid unpair and spp unpair
    #can recieve the signals in any order
    #on success
        #determins if its full pair or disconnection only 
    def async_check(self,status):
        try:
            ...
            logger.error(f"StateMachine DisconnectionState async_check error: {e}")
            raise
...
class ErrorState(QState):
    ...
    def onEntry(self, event):
        try:
            ...
            logger.error(f"StateMachine ErrorState onEntry error: {e}")
    ...
    def onExit(self, event):
        try:
            ...
            logger.error(f"StateMachine ErrorState onExit error: {e}")            
...
class ConnectionState(QState):
    ...
    def onEntry(self, event):
        try:
            ...
            logger.error(f"StateMachine ConnectionState onEntry error: {e}")
    ...
    def onExit(self, event):
        try:
            ...
            logger.error(f"StateMachine ConnectionState onExit error: {e}")
    ...
    def handle_finish(self, status):
        try:
            ...
            logger.error(f"StateMachine ConnectionState handle_finish error: {e}")
    ...
    def async_check(self,status):
        try:
            ...
            logger.error(f"StateMachine ConnectionState async_check error: {e}")
            raise
...
class DeviceSearchState(QState):
    ...
    def onEntry(self, event):
        try:
            ...
            logger.error(f"StateMachine DeviceSearchState onEntry error: {e}")
    ...
    def onExit(self, event):
        try:
            ...
            logger.error(f"StateMachine DeviceSearchState onExit error: {e}")
    ...
    def handle_finish(self, status):
        try:
            ...
            logger.error(f"StateMachine DeviceSearchState handle_finish error: {e}")
    ...
    def device_power_check(self):
        try:
            ...
            logger.error(f"StateMachine DeviceSearchState device_power_check error: {e}")
    ...
    def handle_power_finish(self, res):
        try:
            ...
            logger.error(f"StateMachine DeviceSearchState handle_power_finish error: {e}")
    ...
    def clear_le_controller(self,controller):
        try:
            ...
            logger.error(f"StateMachine DeviceSearchState clear_le_controller error: {e}")
            raise
    ...
    def unify_list(self):
        try:
            ...
            logger.error(f"StateMachine DeviceSearchState unify_list error: {e}")
            raise

    #start the process of looking for the spp device using the btdiscovery dependency
    def spp_fallback(self,mac_str):
        try:
            ...
            logger.error(f"StateMachine DeviceSearchState spp_fallback_finish_handle error: {e}")
            raise

    #ends the process of spp service search
    def spp_fallback_finish_handle(self,message: dict):
        try:
    ...
    def cached_check(self,mac_str):
        try:
            ...
            logger.error(f"StateMachine DeviceSearchState cached_check error: {e}")
            raise

    #if spp service not found
        #check for uuid in cached devices file
        #if not found
            #trigger spp fallback
            #create new entry on pending callbacks
            #assign counter to true, meaning we have yet to find the spp service of a device that has been assign as powered
    def service_null_check(self):
        try:
            ...
            logger.error(f"StateMachine DeviceSearchState service_null_check error: {e}")
            raise
         
    #deals with the return message of the fallback
    #expects a dict with the mac address of the device
    #pops the pending_callbacks entry    
    def fallback_return_result(self, message):
        try:
            ...
            logger.error(f"StateMachine DeviceSearchState fallback_return_result error: {e}")
    ...
    def async_check(self, status):
        try:
            ...
            logger.error(f"StateMachine DeviceSearchState async_check error: {e}")
            raise
...
class FindPortState(QState):
    ...
    def onEntry(self, event):
        try:
            ...
            logger.error(f"StateMachine FindPortState onEntry error: {e}")
    ...
    def onExit(self, event):
        try:
            ...
            logger.error(f"StateMachine FindPortState onExit error: {e}")

    def on_socket_sucess(self):
        try:
            self.pair_success.emit(self.machine.addr)
        except Exception as e:
            logger.error(f"StateMachine FindPortState on_socket_sucess error: {e}")